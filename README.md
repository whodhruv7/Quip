...
**Build 13 — The Magical Hand: Ghost Cursor + care + open-anything** *(latest)*
- **✨ Ghost Cursor — Quip's visible magical hand** — when Quip acts on your screen you SEE it: a glowing theme-colored cursor arcs to the exact spot (quadratic-bezier glide), presses with a squash, **bursts into sparkles on click**, leaves a stardust trail, and carries a caption ("opening youtube…", `typing "hello…"`). Fully click-through (never blocks your mouse), never steals focus, theme-synced (10 themes + companion tints), DPI-aware for 125%/150% scaling, reduced-motion aware. Wired into EVERY real input: click, double/right-click, drag (grab mode), scroll, typing — both direct commands and agent/screen-vision actions. Off-platform honesty: the cursor only performs where real input can actually run (Windows)
- **💧 Care routines** — hydration (45m), eye rest (60m), posture (90m), screen-time (2h) reminders, each with its own synthesized signature sound (droplet / chime / warm triad / rising arpeggio), varied Hinglish-friendly copy, one-at-a-time cooldown, silent at night (23:00–07:30), Settings toggle
- **👀 App watcher** — opens a new app? Quip gently asks "need any help?" with an **Open it** button that focuses the app directly. Rate-limited (5 min), stability-gated, never interrupts Quip's own actions or tasks, never asks about Quip itself, Settings toggle
- **🩹 ErrorBar on every screen** — a small honest strip (companion/panel/full/fullscreen) listing open problems from the Problem Diary with severity dots, occurrence counts, per-item detail, and a **Download report** button (Markdown to Desktop) to send to the developer
- **🕓 Chat history** — new History drawer in the top bar: archived conversations with titles, dates and message counts; tap to reopen any past chat exactly as it was
- **Open-anything round** — **F-29 fixed**: bare domains (`open github.com`) misrouted to open_folder and opened an Explorer error; now they route as websites. App-name normalization hardened ("Chrome!", "(edge)" no longer break matching); Hinglish opens ("youtube kholo") verified by test
- **Understanding → diary** — a request that looks like a task but can't be turned into an action (unusual phrasing or missing capability) is now honestly recorded in the Problem Diary instead of vanishing
- **Companion gate** — structural test proves all six companions ship the complete, consistent definition set
- **548/548 tests** (26 new) · tsc clean · TDZ audit 101 files / 0 hazards · Vite build ✓ · Xvfb launch verified

**Build 12 — Problem Diary + Completion Round (150/150)**

...
exports.buildInstalledAppIndex = buildInstalledAppIndex;
exports.getCachedAppIndex = getCachedAppIndex;
exports.invalidateAppIndex = invalidateAppIndex;
exports.scoreAppMatch = scoreAppMatch;
...
function procNameFromExe(exePath) {
    return node_path_1.default.basename(exePath).replace(/\.exe$/i, "");
}
...
function startMenuDirs() {
    ...
    return [
        appData ? node_path_1.default.join(appData, "Microsoft", "Windows", "Start Menu", "Programs") : "",
        programData ? node_path_1.default.join(programData, "Microsoft", "Windows", "Start Menu", "Programs") : "",
    ].filter(Boolean);
}
function localProgramsDirs() {
    const localAppData = process.env.LOCALAPPDATA ?? "";
    const pf = process.env.ProgramFiles ?? "C:\\Program Files";
    const pf86 = process.env["ProgramFiles(x86)"] ?? "C:\\Program Files (x86)";
    return [
        localAppData ? node_path_1.default.join(localAppData, "Programs") : "",
        pf,
        pf86,
    ].filter(Boolean);
}
...
Get-ChildItem -Path @(${dirList}) -Recurse -Filter *.lnk -ErrorAction SilentlyContinue | ForEach-Object {
  ...
  if ($t.TargetPath) { "$($_.BaseName)|$($t.TargetPath)" }
    ...
    const res = await (0, action_verifier_1.runCapture)(`powershell -NoProfile -ExecutionPolicy Bypass -Command "${ps.replace(/"/g, '\\"').replace(/\r?\n/g, "; ")}"`, 25000);
    ...
    for (const line of res.stdout.split(/\r?\n/)) {
        ...
        if (!name || !target)
            continue;
        if (SKIP_WORDS.test(name) || !target.toLowerCase().endsWith(".exe"))
            continue;
...
async function scanProgramDirs() {
    const apps = [];
        ...
        try {
            ...
            for (const entry of entries) {
                if (!entry.isDirectory())
                    continue;
                if (SKIP_WORDS.test(entry.name))
                    continue;
                const dirPath = node_path_1.default.join(dir, entry.name);
                try {
                    ...
                    if (exe) {
                        const exePath = node_path_1.default.join(dirPath, exe);
...
async function scanUwpApps() {
    const res = await (0, action_verifier_1.runCapture)(`powershell -NoProfile -Command "Get-StartApps | ForEach-Object { if ($_.AppID -like '*!*') { '$($_.Name)|$($_.AppID)' } }"`, 15000);
    if (!res || !res.stdout)
        return [];
    const apps = [];
    for (const line of res.stdout.split(/\r?\n/)) {
        ...
        if (!name || !aumid || SKIP_WORDS.test(name))
            continue;
        const norm = normalizeAppName(name);
        if (apps.some((a) => normalizeAppName(a.name) === norm))
            continue;
...
// ─── Index build + cache ─────────────────────────────────────────────────────
let memCache = null;
async function buildInstalledAppIndex(userDataPath, force = false) {
    // 1. memory cache
    if (!force && memCache && Date.now() - memCache.at < CACHE_TTL_MS)
        return memCache.apps;
    // 2. disk cache
    if (!force && userDataPath) {
        try {
            const p = node_path_1.default.join(userDataPath, CACHE_FILE);
    ...
    if (userDataPath) {
        try {
            node_fs_1.default.writeFileSync(node_path_1.default.join(userDataPath, CACHE_FILE), JSON.stringify({ at: memCache.at, apps: merged }));
        }
        catch {
            /* best effort */
        }
...
function scoreAppMatch(queryNorm, appNameNorm) {
    if (!queryNorm || !appNameNorm)
        return 0;
    if (queryNorm === appNameNorm)
        return 1;
    const aliasFor = COMMON_ALIASES[appNameNorm];
    if (aliasFor && aliasFor.some((a) => a === queryNorm))
        return 0.97;
    const qWords = new Set(queryNorm.split(" ").filter(Boolean));
    const aWords = appNameNorm.split(" ").filter(Boolean);
    let overlap = 0;
    for (const w of qWords) {
        if (aWords.includes(w))
            overlap++;
        else if (aWords.some((aw) => aw.startsWith(w) && w.length >= 3))
            overlap += 0.5;
    }
    const coverage = overlap / Math.max(qWords.size, 1);
    let score = coverage * 0.9;
    if (appNameNorm.includes(queryNorm) && queryNorm.length >= 3)
        score = Math.max(score, 0.85);
    if (aWords.length === 1 && queryNorm === appNameNorm)
        score = 1;
 ...
 */
function resolveApp(query, apps) {
...
async function launchApp(app) {
    ...
    if (app.executable) {
        try {
            if (!node_fs_1.default.existsSync(app.executable)) {
        ...
        const res = await (0, action_verifier_1.runCapture)(`cmd /c start "" "${app.executable}"`, 6000);
        if (res === null) {
            evidence.push("start command failed to spawn");
        }
    }
    else if (app.appUserModelId) {
        const res = await (0, action_verifier_1.runCapture)(`cmd /c start "" "shell:AppsFolder\\${app.appUserModelId.replace(/"/g, "")}"`, 6000);
        if (res === null)
            evidence.push("shell:AppsFolder launch failed");
    }
    else if (app.startCommand) {
        const res = await (0, action_verifier_1.runCapture)(app.startCommand, 6000);
        if (res === null)
            evidence.push("start command failed");
    }
    else {
        return {
            ok: false,
    ...
    const procNames = [app.procName, normalizeAppName(app.name).replace(/\s+/g, "")].filter(Boolean);
    const verification = await (0, action_verifier_1.verifyLaunched)({
        procNames,
        titleHints: [app.name],
        timeoutMs: 9000,
    });
    if (verification.ok)
        return { ...verification, summary: `Opened ${app.name}.` };
    return {
        ...verification,

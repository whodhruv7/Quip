...
exports.parseIntentV2 = parseIntentV2;
// ─── Alias tables ────────────────────────────────────────────────────────────
const APP_HINTS = {
    ...
    for (const key of keys) {
        if (key.includes(" ")) {
            if (text.includes(key))
                return { key, value: table[key] };
        }
    }
    for (const word of words) {
        if (table[word])
            return { key: word, value: table[word] };
    }
    return null;
}
// ─── Normalization ───────────────────────────────────────────────────────────
...
function looksLikeLocalFileQuery(q) {
    return (/\.(pdf|docx?|txt|xlsx?|pptx?|png|jpe?g|gif|mp3|mp4|zip|csv|json|md)\b/i.test(q) ||
...
function parseIntentV2(raw, opts = {}) {
    ...
    const context = opts.context ?? { updatedAt: 0 };
    ...
    const playLead = !!startsWithAny(text, PLAY_WORDS) ||
    ...
    for (const [words, action] of [[CLOSE_WORDS, "close_app"], [FOCUS_WORDS, "focus_app"]]) {
        const verb = startsWithAny(text, words);
        if (verb) {
    ...
    if (typeMatch && !/\bemail|message|mail\b/.test(text)) {
        ...
        if (inMatch)
            content = content.slice(0, inMatch.index).trim();
        return {
            ...base,
            action: "type",
...
//# sourceMappingURL=intent-parser-v2.js.map
...
exports.computeOverlayRect = computeOverlayRect;
exports.computeBrowserBoundsForWorkArea = computeBrowserBoundsForWorkArea;
exports.rectsIntersect = rectsIntersect;
exports.quipOverlayRect = quipOverlayRect;
exports.computeBrowserWindowBounds = computeBrowserWindowBounds;
/** Pure (testable): overlay region the Quip chat panel occupies (right edge). */
function computeOverlayRect(workArea, panelWidth = 440) {
 ...
 */
function computeBrowserBoundsForWorkArea(workArea, chat) {
...
function quipOverlayRect(panelWidth = 440) {
    const { screen } = require("electron");
    const area = screen.getPrimaryDisplay().workArea;
    return computeOverlayRect(area, panelWidth);
}
/** Browser bounds using the real primary display. */
function computeBrowserWindowBounds(overlay) {
    const { screen } = require("electron");
    const area = screen.getPrimaryDisplay().workArea;
    return computeBrowserBoundsForWorkArea(area, overlay);
}
//# sourceMappingURL=window-policy.js.map
...
function extractTopic(raw, extraStops = []) {
...
// ---------------------------------------------------------------------------
function buildPlan(intent, requestId, profile, env) {
    const ctx = { profile, environment: env };
    const subtasks = [];
    let step = 1;
    const add = (capability, description, params, requiresConfirmation) => {
        subtasks.push({
            id: uid(),
            step: step++,
            description,
    ...
    switch (intent.type) {
        case "open_website": {
            ...
            if (site) {
                const url = site.url;
                add("openUrl", `Open ${site.label}`, { url, reason: `Opening ${site.label}` }, false);
        ...
        case "play_media": {
            ...
            const cap = (0, capability_registry_1.resolveCapability)("playMedia", ctx, { query });
            if (!cap.available) {
                isChat = true;
                summary = "";
                break;
            ...
            add("playMedia", `Play "${query}" on ${label}`, cap.implementation.params, false);
        ...
        case "search": {
            ...
            add("webSearch", `Search the web for "${q}"`, { query: q }, false);
        ...
        case "compose_mail": {
            ...
            add("composeMail", `Draft an email${to ? ` to ${to}` : ""}`, { to, subject, body }, true // medium risk — needs confirmation before sending
            );
...
async function executePlan(plan, profile, env, opts = {}) {
    ...
    for (const sub of plan.subtasks) {
        ...
        // Permission gate.
        if (sub.requiresConfirmation && permission_system_1.permissionSystem.requiresConfirmation(sub.capability)) {
        ...
        const resolution = (0, capability_registry_1.resolveCapability)(sub.capability, ctx, sub.params);
        ...
        const result = await (0, tool_executor_1.executeImplementation)(impl, profile);

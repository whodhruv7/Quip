...
function labelFrom(m) {
    if (m.energy < 0.25)
        return "sleepy";
    if (m.focus > 0.75 && m.energy > 0.5)
        return "focused";
    if (m.energy > 0.75 && m.playfulness > 0.6)
        return "playful";
    if (m.energy < 0.4 && m.focus < 0.4)
        return "relaxed";
    if (m.energy < 0.4 && m.warmth < 0.4)
        return "stressed";
    if (m.energy > 0.6)
        return "fresh";
    return "relaxed";
...
class CompanionMoodBrain {
    moods;
    lastUpdate = 0;
    recentUserStress = 0.5;
    recentUserWarmth = 0.5;
    lastEnv = null;
    constructor() {
        this.moods = {
    ...
    observeUserMessage(message) {
        ...
        let warmth = 0;
        if (warmWords.some((w) => lower.includes(w)))
            warmth += 0.4;
    ...
    recompute() {
        ...
        let timeEnergy = 0.5;
        if (hour >= 6 && hour < 11)
            timeEnergy = 0.85; // morning
        else if (hour >= 11 && hour < 14)
            timeEnergy = 0.7; // midday
        else if (hour >= 14 && hour < 17)
            timeEnergy = 0.6; // afternoon
        else if (hour >= 17 && hour < 21)
            timeEnergy = 0.45; // evening
        else
            timeEnergy = 0.25; // night
        // Battery adjustment
        let batteryAdj = 0;
        ...
        for (const id of Object.keys(this.moods)) {
            ...
            const energy = Math.max(0, Math.min(1, timeEnergy * 0.5 + personality.baseEnergy * 0.4 + batteryAdj + stressAdj));
            const warmth = Math.max(0, Math.min(1, personality.baseWarmth * 0.7 + this.recentUserWarmth * 0.3));
    ...
    getAnimationSpeed(id) {
        const m = this.getMood(id);
        // Energy 0 → 0.5x speed, energy 1 → 1.5x speed
        return 0.5 + m.energy;
    }
}
exports.companionMood = new CompanionMoodBrain();
//# sourceMappingURL=companion-mood.js.map
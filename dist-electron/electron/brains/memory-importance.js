...
exports.isPruneCandidate = isPruneCandidate;
exports.scoreAll = scoreAll;
exports.topN = topN;
exports.runPrune = runPrune;
exports.pinMemory = pinMemory;
exports.unpinMemory = unpinMemory;
const TYPE_WEIGHTS = {
    contact: 0.95,
    fact: 0.9,
    preference: 0.7,
...
function scoreMemory(memory, now = Date.now()) {
    ...
    const importanceWeight = memory.importance === "high" ? 1.0 : memory.importance === "medium" ? 0.6 : 0.3;
    const score = typeWeight * sourceWeight * importanceWeight +
        frequencyBoost +
        recencyBoost;
    return Math.max(0, Math.min(1, score));
}
/** Determine if a memory is a prune candidate. */
function isPruneCandidate(memory, score, now = Date.now()) {
...
function pinMemory(memory) {
    return { ...memory, weight: 10, importance: "high" };
}
/** Unpin a memory: resets weight to 1. */
function unpinMemory(memory) {
    return { ...memory, weight: 1 };
}
//# sourceMappingURL=memory-importance.js.map
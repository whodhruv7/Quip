...
export interface CommunicationDNA {
  toneLabel: string;          // "casual", "balanced", "formal"
  preferredLength: string;    // "brief", "moderate", "detailed"
  usesEmoji: boolean;
  prefersCode: boolean;
  styleFacts: string[];       // raw style memories from extractor
  promptFragment: string;     // ready-to-inject text
  updatedAt: number;
}
...
function deriveTone(formality: number): "casual" | "balanced" | "formal" {
  if (formality < 0.35) return "casual";
  if (formality < 0.65) return "balanced";
  return "formal";
}

function deriveLength(words: number): "brief" | "moderate" | "detailed" {
  if (words <= LENGTH_THRESHOLDS.brief) return "brief";
  if (words <= LENGTH_THRESHOLDS.moderate) return "moderate";
  return "detailed";
}

export class CommunicationDNAEngine {
  private cached: CommunicationDNA | null = null;
  private cacheMs = 30_000; // re-compute every 30s max

  /**
   * Compute the DNA from the current user profile + extracted style memories.
   * Returns a cached result if fresh enough.
   */
  compute(profile: UserProfile, memories: MemoryState): CommunicationDNA {
    const now = Date.now();
...
export const communicationDNA = new CommunicationDNAEngine();

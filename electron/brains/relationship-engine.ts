...
export interface UserProfile {
  ...
  preferredResponseLength: number; // in words
  formality: number; // 0 = casual, 1 = formal
  emojiUsage: number; // 0-1 frequency
  humorLevel: number; // 0-1
  wantsCodeInResponses: boolean;
  activeHours: number[]; // 0-23, count per hour
...
function ema(current: number, newValue: number, alpha = EMA_ALPHA): number {
  return current * (1 - alpha) + newValue * alpha;
}

/** Score formality of a message: 0 = casual, 1 = formal. */
function scoreFormality(message: string): number {
  const lower = message.toLowerCase();
  const words = lower.split(/\s+/).length || 1;
  let casualCount = 0;
  let formalCount = 0;
  for (const m of CASUAL_MARKERS) {
    if (lower.includes(m)) casualCount++;
  }
  for (const m of FORMAL_MARKERS) {
    if (lower.includes(m)) formalCount++;
  }
  // Punctuation: exclamations = casual, semicolons = formal
  if (/\!/.test(message)) casualCount += 0.5;
  if (/;/.test(message)) formalCount += 0.5;

  if (casualCount === 0 && formalCount === 0) return 0.5; // neutral
  const total = casualCount + formalCount;
  return Math.max(0, Math.min(1, formalCount / total));
}

/** Count emojis in a message. */
function countEmojis(message: string): number {
...
class RelationshipEngine {
  ...
  observeAssistantResponse(response: string, userFeedback?: "too_long" | "too_short" | "good"): void {
    const wordCount = response.split(/\s+/).length;

    if (userFeedback === "too_long") {
      this.profile.preferredResponseLength = Math.max(
        20,
        Math.round(this.profile.preferredResponseLength * 0.7)
      );
    } else if (userFeedback === "too_short") {
      this.profile.preferredResponseLength = Math.min(
        500,
        Math.round(this.profile.preferredResponseLength * 1.3)
      );
    } else {
      // Slowly drift toward the response length the user is engaging with
      this.profile.preferredResponseLength = Math.round(
        ema(this.profile.preferredResponseLength, wordCount)
      );
    }
    this.save();
  }
  ...
  observeCodePreference(message: string): void {
    const lower = message.toLowerCase();
  ...
  getStyleGuide(): string {
    ...
    const formalityLabel =
      p.formality < 0.3 ? "casual" : p.formality < 0.7 ? "balanced" : "formal";

    const emojiLabel =
      p.emojiUsage < 0.1 ? "rarely" : p.emojiUsage < 0.4 ? "occasionally" : "frequently";

    const humorLabel =
      p.humorLevel < 0.3 ? "serious" : p.humorLevel < 0.6 ? "light humor ok" : "humor welcome";
...
- Code in responses: ${p.wantsCodeInResponses ? "yes" : "no"}${topTopicsStr}`;
  ...
  override(patch: Partial<UserProfile>): void {

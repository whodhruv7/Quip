...
class Orchestrator {
  ...
  async execute(
    ...
    // ─── Step 1: Parse intent ──────────────────────────────────────────────
    const intent = parseIntentV2(command);
    ...
    if (permissionSystem.planNeedsApproval()) {
    ...
    for (let i = 0; i < intent.steps.length; i++) {
      ...
      if (permissionSystem.getMode() === "ask_every_time") {
        const approved = await this.requestApproval(
          step.description,
          [step.description],
          opts.onApprovalRequest
  ...
  private async executeWithRetry(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      const result = await executeTool(step.action, step.params, ctx);
  ...
  private async requestApproval(
    ...
    if (!callback) return true; // no callback = auto-approve (shouldn't happen)

    const result = await permissionSystem.requestApproval(title, steps);
